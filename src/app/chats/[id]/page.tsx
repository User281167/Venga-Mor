...
import { Avatar, Button, Card, Flex, Heading, Section, Text, TextField } from '@radix-ui/themes';
...
export default function ChatPage({ params }: { params: { id: string } }) {
  ...
  const handleSend = () => {
    ...
    const currentNode = (chatFlow.nodos as Record<string, ChatNode>)[currentNodeKey];
    if (currentNode.siguiente_automatico) {
        setInputDisabled(true);
        setTimeout(() => processNextNode(currentNode.siguiente_automatico!, userInput), 500);
    }
  ...
  const processNextNode = (nextNodeKey: string, previousInput?: string) => {
    const nextNode: ChatNode = (chatFlow.nodos as Record<string, ChatNode>)[nextNodeKey];
  ...
  return (
    <Section className="py-8 px-4 h-[calc(100vh-80px)] flex flex-col">
      <Card className="max-w-2xl w-full mx-auto bg-card/80 flex-grow flex flex-col">
        <Flex p="4" align="center" gap="4" className="border-b border-border">
          <Avatar
            ...
            radius="full"
          />
          <Heading as="h1" className="text-2xl">{escort.name}</Heading>
        </Flex>

        <Flex direction="column" gap="3" className="p-4 flex-grow overflow-y-auto">
            ...
            <div key={index} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`p-3 rounded-lg max-w-[80%] ${msg.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-secondary'}`}>
                 ...
                 {msg.sender === 'bot' && msg.options && (
                    <Flex direction="column" gap="2" mt="2" className="max-w-[80%] w-full">
                        {msg.options.map(opt => (
                            <Button key={opt.next} variant='outline' onClick={() => handleOptionClick(opt)}>
                                {opt.label}
                            </Button>
                        ))}
                    </Flex>
        ...
        <Flex p="4" gap="3" align="center" className="border-t border-border">
          <TextField.Root 
            className="flex-grow" 
            placeholder={inputDisabled ? "Selecciona una opción..." : "Escribe un mensaje..."}
            value={userInput}
