...
export default function ClipsPage() {
    ...
    return (
        <Section id="clips-container" className="relative h-[calc(100vh-64px)] w-full overflow-y-auto scroll-snap-y-mandatory bg-black">
            <div className="relative z-10 flex flex-col h-full w-full max-w-md mx-auto">
                {posts.map((post) => (
                    <div key={post.id} className="relative h-[calc(100vh-64px)] w-full flex-shrink-0 scroll-snap-start flex items-center justify-center">
                        <ClipItem post={post} />
                    </div>
                ))}
                ...
                {posts.length === 0 && !isLoading && (
                     <div className="relative h-[calc(100vh-64px)] w-full flex-shrink-0 scroll-snap-start flex items-center justify-center text-white text-center p-4">
                        <div>
                            ...
                            <Text as="p" color="gray">Los colaboradores aún no han publicado nada.</Text>
                        </div>
                    </div>
                )}
